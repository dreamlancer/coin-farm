...
const IfoCardContribute: React.FC<Props> = ({
  ...
  const [offeringTokenBalance, setOfferingTokenBalance] = useState(
    new BigNumber(0)
  );
  ...
  useEffect(() => {
    const fetch = async () => {
      const balance = new BigNumber(
        await contract.methods.getOfferingAmount(account).call()
      );
  ...
  const percentOfUserContribution = new BigNumber(userInfo.amount)
    .div(raisingAmount)
    .times(100);
