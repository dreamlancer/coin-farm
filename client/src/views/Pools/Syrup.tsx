...
const Farm: React.FC = () => {
  ...
  const poolsWithApy = pools.map((pool) => {
    ...
    const stakingTokenFarm = farms.find(
      (s) => s.tokenSymbol === pool.stakingTokenName
    );

    // tmp mulitplier to support ETH farms
    // Will be removed after the price api
    const tempMultiplier =
      stakingTokenFarm?.quoteTokenSymbol === "ETH" ? ethPriceBnb : 1;

    // /!\ Assume that the farm quote price is BNB
    const stakingTokenPriceInBNB = isBnbPool
      ? new BigNumber(1)
      : new BigNumber(stakingTokenFarm?.tokenPriceVsQuote).times(
          tempMultiplier
        );
    ...
    return {
      ...pool,
      isFinished:
        pool.sousId === 0 ? false : pool.isFinished || block > pool.endBlock,
      apy,
    };
  });

  const [finishedPools, openPools] = partition(
    poolsWithApy,
